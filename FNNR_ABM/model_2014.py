...
def make_land_agents_2014(self):
    ...
    for hh_row in agents:  # from excel_import
        ...
        for i in range(len(return_values(hh_row, 'non_gtgp_rice_mu'))):
            try:
                self.land_area = return_values(hh_row, 'non_gtgp_rice_mu')[i]
            except:
                pass
            if self.land_area != 0:
                self.land_type = 0
            try:
                self.non_gtgp_output = return_values(hh_row, 'non_gtgp_output')[i)]
            except:
                pass
            self.land_time = return_values(hh_row, 'non_gtgp_travel_time')[i]
            try:
                self.plant_type = return_values(hh_row, 'non_gtgp_plant_type')[i]
            except:
                pass
            try:
                self.land_type = return_values(hh_row, 'non_gtgp_land_type')[i]
            except:
                pass
