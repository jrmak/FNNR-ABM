...
class IndividualAgent(Agent):
    ...
    def birth(self):
        ...
        if self.marriage == 1 and self.gender == 2 and self.age < 55    \
                and self.individual_id not in out_migrants_list and birth_flag_list != []:
    ...
    def death(self):
        ...
        if random() < self.death_rate:
            self.death_flag += 1
        if self.age > 65 and self.death_flag > 0 and self.individual_id not in out_migrants_list    \
                and self.individual_id not in death_list:
                    self.hh_id = 'Dead'
                    if '2014' not in self.individual_id:
    ...
    def out_migration(self):
        ...
        self.re_mig_prob = 0  # pre-set in pseudocode
    ...
    def step(self):
        ...
        if self.hh_id != 'Dead':
            ...
            self.age += 1
        #if '2014' not in self.individual_id:
            #print(sum(num_mig_list)/94, sum(num_labor_list)/94,
            #      sum(hh_size_list)/94)
        ...
        if int(self.step_counter) < 5:
            ...
            if str(self.individual_id)[-1] == 'a' and self.hh_id != 'Dead'  \
                ...
                and self.hh_id != 'Migrated' and self.hh_id not in hhlist:
                    ...
                    if '2014' not in self.individual_id:
                        save(self.step_counter, self.current_year, self.individual_id[:-1], self.num_labor, self.num_mig,
                             self.hh_size, household_income[self.hh_row - 1])
                    else:
                        save(self.step_counter, self.current_year, self.individual_id[:-1], self.num_labor, self.num_mig,
                             self.hh_size, household_income[self.hh_row - 1])
                ...
                and self.hh_id != 'Migrated' and self.hh_id not in hhlist:
                    ...
                    if '2014' not in self.individual_id:
                        save(self.step_counter, self.current_year, self.individual_id[:-1], self.num_labor, self.num_mig,
                             self.hh_size, household_income_2014[self.hh_row - 1])
                    else:
                        save(self.step_counter, self.current_year, self.individual_id[:-1], self.num_labor, self.num_mig,
                             self.hh_size, household_income[self.hh_row - 1])
        try:
