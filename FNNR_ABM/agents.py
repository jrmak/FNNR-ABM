...
class HouseholdAgent(Agent):  # child class of Mesa's generic Agent class
    ...
    def step(self):
        ...
        #          self.hh_size)
        if self.hh_step_counter == 0:  # initializes household income
...
class LandParcelAgent(Agent):
    ...
    def non_gtgp_count2(self, nongtgplist, gtgplist):
        """Counts non-GTGP land parcels for graphing"""
        if self.gtgp_enrolled == 0 and self.unique_id not in nongtgplist:
            nongtgplist.append(self.unique_id)
        return len(nongtgplist)
    ...
    def re_migration(self):
        ...
        if self.individual_id in out_migrants_list or self.individual_id in out_migrants_list_2014:
            ...
            if random() < re_mig_prob:
                household_income[self.hh_row - 3] = household_income[self.hh_row - 3] - self.remittance
                self.remittance = 0
